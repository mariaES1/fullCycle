...
func (c *Course) Create(name string, description, categoryID string) (*Course, error) {
	id := uuid.New().String()
...
func (c *Course) FindByCategoryID(cartegoryID string) ([]Course, error) {
	rows, err := c.db.Query("SELECT id, name, description, category_id FROM courses WHERE category_id = $1", cartegoryID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
